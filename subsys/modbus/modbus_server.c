...
static bool mbs_fc01_coil_read(struct modbus_context *ctx)
{
	...
	while (coil_cntr < coil_qty) {
		...
		err = ctx->mbs_user_cb->coil_rd(coil_addr, &coil_state, ctx->rawcb.user_data);
		if (err != 0) {
...
static bool mbs_fc02_di_read(struct modbus_context *ctx)
{
	...
	while (di_cntr < di_qty) {
		...
		err = ctx->mbs_user_cb->discrete_input_rd(di_addr, &di_state, ctx->rawcb.user_data);
		if (err != 0) {
...
static bool mbs_fc03_hreg_read(struct modbus_context *ctx)
{
	...
	while (reg_qty > 0) {
		if (reg_addr < MODBUS_FP_EXTENSIONS_ADDR) {
			...
			/* Read integer register */
			err = ctx->mbs_user_cb->holding_reg_rd(reg_addr, &reg, ctx->rawcb.user_data);
			if (err == 0) {
			...
			/* Read floating-point register */
			err = ctx->mbs_user_cb->holding_reg_rd_fp(reg_addr, &fp, ctx->rawcb.user_data);
			if (err == 0) {
...
static bool mbs_fc04_inreg_read(struct modbus_context *ctx)
{
	...
	while (reg_qty > 0) {
		if (reg_addr < MODBUS_FP_EXTENSIONS_ADDR) {
			...
			/* Read integer register */
			err = ctx->mbs_user_cb->input_reg_rd(reg_addr, &reg, ctx->rawcb.user_data);
			if (err == 0) {
				sys_put_be16(reg, presp);
				presp += sizeof(uint16_t);
			}
			...
			/* Read floating-point register */
			err = ctx->mbs_user_cb->input_reg_rd_fp(reg_addr, &fp, ctx->rawcb.user_data);
			if (err == 0) {
				memcpy(&reg, &fp, sizeof(reg));
				sys_put_be32(reg, presp);
				presp += sizeof(uint32_t);
...
static bool mbs_fc05_coil_write(struct modbus_context *ctx)
{
	...
	err = ctx->mbs_user_cb->coil_wr(coil_addr, coil_state, ctx->rawcb.user_data);
...
static bool mbs_fc06_hreg_write(struct modbus_context *ctx)
{
	...
	err = ctx->mbs_user_cb->holding_reg_wr(reg_addr, reg_val, ctx->rawcb.user_data);
...
static bool mbs_fc15_coils_write(struct modbus_context *ctx)
{
	...
	while (coil_cntr < coil_qty) {
		...
		err = ctx->mbs_user_cb->coil_wr(coil_addr + coil_cntr,
						coil_state, ctx->rawcb.user_data);
...
static bool mbs_fc16_hregs_write(struct modbus_context *ctx)
{
	...
	for (uint16_t reg_cntr = 0; reg_cntr < reg_qty; reg_cntr++) {
		...
		if ((reg_addr < MODBUS_FP_EXTENSIONS_ADDR) ||
			...
			err = ctx->mbs_user_cb->holding_reg_wr(addr, reg_val, ctx->rawcb.user_data);
		} else {
			...
			err = ctx->mbs_user_cb->holding_reg_wr_fp(addr, fp, ctx->rawcb.user_data);
		}
